function main(training,predictions,C,N,eps,epsilon,kernel, param_kernel, lambda_0, max_iter,tol)
    ...
    [sol, iter_true]= PGM(Q,q,epsilon,C,lambda_0,N,max_iter,tol);
    ...
    x=1:(iter_true);
    semilogy(x, errore, 'k', 'DisplayName', sprintf('Convergenza sperimentale'));% errore in funzione del numero di iterazioni
    legend('show');
    title('Grafico errore f ed f*');
    xlabel('Iterazioni k');
